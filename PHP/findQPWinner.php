...
if(isset ($_POST['sid']) && isset ($_POST['mid'])) {
    if(!empty ($_POST['sid']) && !empty ($_POST['mid']) ) {
        ...
        //your team*********************************************
        echo "<table width='95%'><tr><td width='50%'>";
            ...
            if(mysql_num_rows($ResultSet_to_find_team)==1) {
                echo '<div class="black-header font-containt-bold"><center>YOUR TEAM</center></div>';
                ...
                echo "<div align='center' style='border:1px solid silver;'>
                           <table width=100% style='color:#454346;font-size:12px;padding-left:10px;'>
                             <tr>
                                <td>MANAGER :  $d_firstname $d_lastname &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; LAST UPDATED : $last_modified1</td>
                             </tr>
                                 <tr>
        ...
        //winner***************************************
        echo "<td width='50%'>";
        $query_to_fing_first_rank="SELECT * FROM daily_challenge_eleven_player WHERE sid=$sid AND matchid=$mid
        HAVING daily_point=(SELECT MAX(daily_point) from daily_challenge_eleven_player where sid=$sid AND matchid=$mid)
        ...
        if($ResultSet_to_fing_first_rank=mysql_query($query_to_fing_first_rank)) {
            if(mysql_num_rows($ResultSet_to_fing_first_rank)==1) {
                echo '<div class="black-header font-containt-bold"><center> WINNER OF QUICK PLAY MATCH</center></div>';
